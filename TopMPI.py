...
def main():
    ...
    if not args.decoy and args.spectrum_cutoff_type == "FDR":
        ...
        exit(1)
    elif args.skip_html_folder:
        print("TopMPI is currenty dependent on the HTML files generated by TopPIC. Please remove the argument '-g' from the command.")
        exit(1)
    ...
    for input_file in input_files:
        ...
        merge.main([new_sub_dir, filterbyFeature, keepDecoys, "-t", args.spectrum_cutoff_type, "-v", str(args.spectrum_cutoff_value), "-T", args.proteoform_cutoff_type, "-V", str(args.proteoform_cutoff_value)])


    if args.combined_file_name:
        filterbyFeature = str(not args.no_topfd_feature)
        keepDecoys = str(args.keep_decoy_ids)
        combine.combine(combined_name=args.combined_file_name, input_files=input_files, filterbyFeature=filterbyFeature, keepDecoys=keepDecoys, spectrumcutofftype=args.spectrum_cutoff_type, spectrumcutoffvalue=args.spectrum_cutoff_value, proteoformcutofftype=args.proteoform_cutoff_type, proteoformcutoffvalue=args.proteoform_cutoff_value)
