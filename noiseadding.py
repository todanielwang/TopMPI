...
def main():
    ...
    for scan in candidate_dict.keys():
        ...
        diff = 1.5
        possibleList = [x for x in nonmulti_dict.keys() if (x != scan) and (float(nonmulti_dict[x].header.pre_mz_list[0]) - float(spec.header.pre_mz_list[0]) < diff) and (len(nonmulti_dict[x].peak_list) >= math.ceil(length * topnoise)) and (prsm_df[prsm_df["Scan(s)"] == int(x)].iloc[0]["Protein accession"] != protein)]
        ...
        new_noise_peak_list, peakcount = removePeaks(noise_peak_list, proteoform)

        if (len(new_noise_peak_list) < length * topnoise):
            continue

        if peakcount > 0:
            averagepeaklist.append(peakcount)
            numremovedpeaks += 1
    ...
    print("We removed peaks from " + str(numremovedpeaks) + " noise spctra and removed an average of " + str(np.average(averagepeaklist)) + " peaks.")
    print("We now have " + str(len(outputList[0])) + " spectra left")

    for idx in range(0, 11):
        filenamelist = args[0].rsplit(".", 1)
        read_msalign.write_spec_file(filenamelist[0] + str(idx * 0.2) + filenamelist[1], outputList[idx])
