...
def main():
    ...
    for x in sorted(os.listdir(args[3]), key=numericalSort):
        with open(args[3] + x) as file:
            ...
            if (prevscan == toppicscan):
                if (len(prevMatchList) != 0 and prevMatchList != matchList):
                    spec = Spectrum.get_spec(copy.deepcopy(spec_list[curr_spec].header), prevMassList)
                    spec.header.spec_id += 1
                    spec_list.insert(curr_spec + 1, spec)
                    curr_spec += 1
                    count += 1
                else:
                    continue
            prevscan = toppicscan
            prevMatchList = copy.deepcopy(matchList)
            prevMassList = copy.deepcopy(spec_list[curr_spec].peak_list)
