...
def main():
    ...
    inputdf.loc[mask, ["A+B_1 peaks", "A+B_1 E-value", "A+B_1_mass"]] = 0, 1, 0

    mask = (inputdf['A+B_1'] != "-") & (inputdf['A+B_1'] == inputdf["A+B_2"]) & (inputdf["A+B_1 peaks"] >= inputdf["B+A_1 peaks"])
    inputdf.loc[mask, "A+B_2"] = "-"
    inputdf.loc[mask, ["A+B_2 peaks", "A+B_2 E-value", "A+B_2_mass"]] = 0, 1, 0

    mask = (inputdf['B+A_1'] != "-") & (inputdf['B+A_1'] == inputdf["B+A_2"]) & (inputdf["A+B_1 peaks"] > inputdf["B+A_1 peaks"])
    inputdf.loc[mask, "B+A_1"] = "-"
    inputdf.loc[mask, ["B+A_1 peaks", "B+A_1 E-value", "B+A_1_mass"]] = 0, 1, 0

    mask = (inputdf['B+A_1'] != "-") & (inputdf['B+A_1'] == inputdf["B+A_2"]) & (inputdf["A+B_1 peaks"] <= inputdf["B+A_1 peaks"])
    inputdf.loc[mask, "B+A_2"] = "-"
    inputdf.loc[mask, ["B+A_2 peaks", "B+A_2 E-value", "B+A_2_mass"]] = 0, 1, 0

    inputdf["choice"] = "-"

    # mask = ((inputdf["A+B_1 E-value"] == inputdf["B+A_1 E-value"]) & (inputdf["A+B_1_mass"] == inputdf["B+A_1_mass"]))
    # inputdf.loc[mask, "choice"] = "Only one feature"

    # mask = ((inputdf["A+B_1 E-value"] == inputdf["B+A_2 E-value"]) & (inputdf["A+B_1_mass"] == inputdf["B+A_2_mass"]))
    # inputdf.loc[mask, "choice"] = "Only one real identification"
